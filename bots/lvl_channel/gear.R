# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Движок cтратегии 'level channel':
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#
###
#' Функция движка стратегии 'level channel':
#' 
...
#' @param str_args Лист с параметрами стратегии
    #' prcDiap Диапазон канала
    #' coefStop Коэф. стопа
    #' countDay Количество дней для усреднения
    #' k_mm Коэфф. ММ
#'
#' @return DATA Лист с данными отработки и данные сделок
#'
#' @export
StrategyGear.lvl_channel  <- function(ohlc_args,
                                      trade_args,
                                      str_args) {
    ### Проверка входных данных
    if (!all(c('ohlc', 'from_date', 'to_date', 'ticker') %in% names(ohlc_args))) {
        stop(paste0('ERROR(StrGear.lvl_channel): Input ohlc_args error!!!'))
    }
    if (!all(c('balance_start', 'balance_operating', 'reinvest', 
        'slips', 'commiss', 'return_type',
        'expiration_date', 'unwanted_event_filter', 'unwanted_event_date',
        'tick_price', 'tick_value', 'gap_filter', 'expiration_filter', 
        'fix_profit', 'fix_profit.period', 'fix_profit.k', 'fix_profit.at', 'trade_handler') %in% names(trade_args))) {
        stop(paste0('ERROR(StrGear.lvl_channel): Input trade_args error!!!'))
    }
    if (!all(c('prsDiap_', 'coefStop_', 'countDay_', 'k_mm') %in% names(str_args))) {
        stop(paste0('ERROR(StrGear.lvl_channel): Input str_args error!!!'))
    }
    ...
    if (ohlc_args$lookback == TRUE) {
        ...
        ohlc_args$ohlc <- do.call('Subset_TradeOHLC', ohlc_args)
    }
    ...
    # cat(
    #     'lvl_channelStrategy INFO:    Start lvl_channelStrategy with parameters:', '\n',
    ...
    #     'lvl_channelStrategy INFO:    Start StrategyData Calculation...', '\n'
    # )
    ...
    DATA <- 
        AddIndicator.lvl_channel(DATA, ohlc_args, str_args) %>% 
        na.omit(.)
    # добавление сигналов 
    DATA <- AddSignal.lvl_channel(DATA, ohlc_args, trade_args, str_args)
    # выделение ордеров
    DATA <- AddPosition.lvl_channel(DATA, ohlc_args)
    if (is.null(DATA)) {
        return(list(data = NULL, state = NULL))
    }
    
    ### STATE-таблица
    #cat('lvl_channelStrategy INFO:    Build state.table...', '\n')
    DATA <- AddStateTable(DATA)
        
    # если сделок нет, то 
    if (is.null(DATA[[2]])) {
        #message('WARNING(StrGear_lvl_channel): No trades Here!!!', '\n')
        return(list(data = NULL, state = NULL))
    }
    
    ### Расчёт цен инструментов на action'ах
    DATA <- AddPrice(DATA, AddPrice.lvl_channel, ohlc_args, trade_args)
    
    ### Расчёт return'ов позиций и состояний
    #cat('lvl_channelStrategy INFO:    CalcReturns for data & states...', '\n')             
    DATA <- AddReturn.lvl_channel(DATA, trade_args)
    ...
    if (trade_args$trade_handler == 'standalone') {
        #cat('lvl_channelStrategy INFO:    TradeHandler Start...', '\n')
        DATA <- TradeHandler.lvl_channel(DATA, ohlc_args, trade_args, str_args)
    } else {
        # если бот - часть пакетной торговли - то используется пакетный обработчик сделок
        #cat('lvl_channelStrategy INFO:    Send DATA to basket handler...', '\n')
    }
    #
    return(DATA)
}
#
# ----------------------------------------------------------------------------------------------------------------------
# Компоненты движка cтратегии 'level channel':
# ----------------------------------------------------------------------------------------------------------------------
#
#' Функция для расчёта индикаторов "level channel"
#'
...
#'
#' @export
AddIndicator.lvl_channel <- function(x, ohlc_args, str_args) {
    # вычисление дневных диапазонов
    x$daydiap <- CalcIndicator.chlvl_daydiap(x, ohlc_args)
    # вычисление уровней
    x$level <- CalcIndicator.chlvl_level(x, ohlc_args, str_args)
    x$up_level <- x$level + str_args$prcDiap * x$daydiap
    x$dn_level <- x$level - str_args$prcDiap * x$daydiap
    #    
    return(x)
}
# 
#' Функция для расчёта торговых сигналов "level channel"
#'
...
#'
#' @export
AddSignal.lvl_channel <- function(x, ohlc_args, trade_args, str_args) {
...
#'
AddPosition.lvl_channel <- function(x, ohlc_args) {
            ...
            if (!is.na(sig_close_row) & sig_close_row != 1) {
                ...
                if (sig_close_row == 1) {
                    ...
                    if(temp$raw_open[2] != 0) {
                        ...
                        if (sig_type == 1) {
                            temp$sig_close.long[2] <- NA
                            sig_close_row <- which(!is.na(temp$sig_close.long))[1]
                        } else {
                            temp$sig_close.short[2] <- NA
                            sig_close_row <- which(!is.na(temp$sig_close.short))[1]
                        } 
                        if (!is.na(sig_close_row)) {
    ...
    if (!any(x$pos != 0)) {
        message('WARNING(lvl_channelStrategy_gear): No trades Here!!!', '\n')
        return(NULL)
    }
    #
    ## Ряд номеров позиций 
...
#'
#' @export 
AddPrice.lvl_channel <- function(x, ohlc) {
...
#' 
#' @export
AddReturn.lvl_channel <- function(x, trade_args) {
...
#'
#' @export
TradeHandler.lvl_channel <- function(data, 
                                 ohlc_args, 
                                 trade_args, 
                                 str_args) {
    #
...
#
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Функции для оптимизации стратегии "level channel":
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#
###
...
#'
#' @export
CalcVarList.lvl_channel <- function(DCI = 20:150, step_DCI = 5,
